...
function buildCertificateEmail(data: {
  ...
  const categoryRows = Object.entries(data.categoryBreakdown)
    .map(([cat, val]) => {
      ...
      return (
        ...
        '</div>' +
        '</td>' +
        '<td style="padding:8px 12px; color:' + barColor + '; font-size:13px; font-weight:bold; border-bottom:1px solid #1e293b; text-align:right;">' + val.correct + '/' + val.total + ' (' + pct + '%)</td>' +
        '</tr>'
      );
    })
    .join('');
  ...
  return '<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>شهادة نتيجة اختبار - Knowlytics Hub</title></head>' +
    ...
    (categoryRows ? (
      ...
      '<th style="padding:8px 12px;color:#64748b;font-size:11px;font-weight:600;border-bottom:1px solid #1e293b;width:50%;">التقدم</th>' +
      '<th style="text-align:right;padding:8px 12px;color:#64748b;font-size:11px;font-weight:600;border-bottom:1px solid #1e293b;">النتيجة</th>' +
      '</tr></thead>' +
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, email, phone, assessmentType, score, total, percentage, level, weakAreas, categoryBreakdown } = body;
      ...
      html: `
        <div dir="rtl" style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;border:1px solid #e5e7eb;border-radius:8px;">
          ...
          <table style="width:100%;border-collapse:collapse;margin-top:16px;">
            <tr style="background:#f8fafc;"><td style="padding:10px 12px;font-weight:bold;width:35%;border:1px solid #e5e7eb;">الاسم</td><td style="padding:10px 12px;border:1px solid #e5e7eb;">${name}</td></tr>
            <tr><td style="padding:10px 12px;font-weight:bold;border:1px solid #e5e7eb;">البريد الإلكتروني</td><td style="padding:10px 12px;border:1px solid #e5e7eb;">${email}</td></tr>
            <tr style="background:#f8fafc;"><td style="padding:10px 12px;font-weight:bold;border:1px solid #e5e7eb;">الهاتف</td><td style="padding:10px 12px;border:1px solid #e5e7eb;">${phone || "—"}</td></tr>
            <tr><td style="padding:10px 12px;font-weight:bold;border:1px solid #e5e7eb;">الاختبار</td><td style="padding:10px 12px;border:1px solid #e5e7eb;">${assessmentType || "—"}</td></tr>
            <tr style="background:#f8fafc;"><td style="padding:10px 12px;font-weight:bold;border:1px solid #e5e7eb;">النتيجة</td><td style="padding:10px 12px;border:1px solid #e5e7eb;">${score}/${total} (${percentage}%)</td></tr>
            <tr><td style="padding:10px 12px;font-weight:bold;border:1px solid #e5e7eb;">المستوى</td><td style="padding:10px 12px;border:1px solid #e5e7eb;">${finalLevel}</td></tr>
            <tr style="background:#f8fafc;"><td style="padding:10px 12px;font-weight:bold;border:1px solid #e5e7eb;">المجالات الضعيفة</td><td style="padding:10px 12px;border:1px solid #e5e7eb;">${weakAreasArr.join(", ") || "—"}</td></tr>
          </table>
